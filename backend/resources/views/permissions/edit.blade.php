...
@section('title', __('common.edit_permission'))

@section('breadcrumb')
    <x-breadcrumb :items="[
        ['label' => __('common.settings')],
        ['label' => __('common.permissions'), 'url' => route('permissions.index')],
        ['label' => __('common.edit_permission')],
    ]" />
@endsection
    ...
    <div>
        <div class="flex items-center justify-between gap-4 mb-6">
            <nav class="text-sm text-slate-500 dark:text-slate-400">
                ...
                <span class="mx-1">/</span>
                <span class="text-slate-700 dark:text-slate-300">{{ __('common.edit_permission') }}</span>
            </nav>
        ...
        <form method="POST" action="{{ route('permissions.update', $permission) }}"
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $permission->name) }}" required autocomplete="off"
                ...
                <p class="mt-1 text-xs text-slate-600 dark:text-slate-300">
                    <span class="font-medium">{{ __('common.permission_ui_preview') }}:</span>
                    {{ \App\Support\PermissionDisplay::label($permission->name) }}
                </p>
            </div>
