    ...
    <div class="table-wrapper">
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50 dark:bg-slate-800/60">
                <tr>
                    <th class="table-header">{{ __('company.company') }}</th>
                    <th class="table-header">{{ __('company.email') }}</th>
                    <th class="table-header">{{ __('company.phone') }}</th>
                    <th class="table-header">{{ __('company.status') }}</th>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                            ...
                            <div class="relative z-10 inline-block text-left pt-0.5" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.outside="open = false" x-cloak
                                    ...
                                    <form method="POST" action="{{ route('companies.destroy', $company) }}" class="block"
                                          onsubmit="return confirm('{{ __('common.are_you_sure') }} {{ __('company.delete_company') }}?')" novalidate>
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('company.no_companies_found') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
