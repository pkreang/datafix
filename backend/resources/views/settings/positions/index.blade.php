...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ __('common.position_list') }}</h2>
        <a href="{{ route('settings.positions.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($positions as $position)
                    <tr>
                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $position->name }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $position->code }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $position->description ?: '-' }}</td>
                            ...
                            <div class="relative inline-block text-left" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.outside="open = false" x-cloak
                                    ...
                                    <form method="POST" action="{{ route('settings.positions.destroy', $position) }}"
                                          onsubmit="return confirm('{{ __('common.delete_confirm_msg', ['name' => $position->name]) }}')">
                                        @csrf
                                        @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('common.no_data') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
