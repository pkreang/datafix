...
<div x-data="{ search: '' }">
    <div class="flex items-center justify-between mb-2">
        <div>
            <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Dashboards</h2>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">ตัวออกแบบ Dashboard</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $totalDashboards }} dashboard(s)</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('settings.dashboards.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-800/80">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Widgets</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    >
                        <td class="px-6 py-3 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{{ $dashboard->name }}</p>
                                    @if($dashboard->description)
                                        <p class="text-xs text-gray-400 dark:text-gray-500 truncate">{{ Str::limit($dashboard->description, 60) }}</p>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-3 whitespace-nowrap">
                            @if($dashboard->visibility === 'permission')
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400">
                            ...
                            <div class="relative inline-block text-left" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.outside="open = false" x-cloak
                                    ...
                                    <form method="POST" action="{{ route('settings.dashboards.destroy', $dashboard) }}"
                                          onsubmit="return confirm('{{ __('common.delete_confirm_msg', ['name' => $dashboard->name]) }}')">
                                        @csrf
                                        @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('common.no_data') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
