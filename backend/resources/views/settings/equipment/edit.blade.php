...
<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ __('common.edit_equipment_category') }}</h2>
    ...
    <form method="POST" action="{{ route('settings.equipment.update', $equipmentCategory) }}">
        @csrf
        @method('PUT')
        <div class="bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <h3 class="text-base font-semibold text-gray-800 dark:text-gray-200 mb-4">{{ __('common.equipment_categories') }}</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="2" maxlength="1000"
                              class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-y bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 @error('description') border-red-400 @enderror">{{ old('description', $equipmentCategory->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $equipmentCategory->is_active) ? 'checked' : '' }}
        ...
        <div class="flex items-center justify-end pt-2 pb-4">
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('settings.equipment.index') }}"
