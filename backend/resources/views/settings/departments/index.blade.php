...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ __('common.department_list') }}</h2>
        <a href="{{ route('settings.departments.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">
    ...
    <div class="bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-visible">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-800/80">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{{ __('common.name') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{{ __('common.code') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{{ __('common.remark') }}</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($departments as $department)
                    <tr>
                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $department->name }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $department->code }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $department->description ?: '-' }}</td>
                            ...
                            <div class="relative inline-block text-left" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.outside="open = false" x-cloak
                                    ...
                                    <form method="POST" action="{{ route('settings.departments.destroy', $department) }}"
                                          onsubmit="return confirm('{{ __('common.delete_confirm_msg', ['name' => $department->name]) }}')">
                                        @csrf
                                        @method('DELETE')
