        ...
        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            <thead class="bg-slate-50 dark:bg-slate-800/60">
                <tr>
                    ...
                    <th class="table-header">{{ __('common.building') }}</th>
                    <th class="table-header">{{ __('common.floor') }}</th>
                    <th class="table-header">{{ __('common.zone') }}</th>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                @forelse ($locations as $location)
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-150">
                        <td class="px-4 py-2 whitespace-nowrap text-sm font-medium text-slate-900 dark:text-slate-100">{{ $location->name }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{{ $location->code }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{{ $location->building ?? '—' }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{{ $location->floor ?? '—' }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{{ $location->zone ?? '—' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('common.no_equipment_locations') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
