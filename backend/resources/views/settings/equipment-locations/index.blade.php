...
<div>
    <div class="flex items-center justify-between mb-2">
        ...
        <a href="{{ route('settings.equipment-locations.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            {{ __('common.add_equipment_location') }}
        </a>
    </div>

    <form method="GET" action="{{ route('settings.equipment-locations.index') }}" class="mb-5">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50 dark:bg-gray-800/80">
                <tr>
                    ...
                    <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('common.building') }}</th>
                    <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('common.floor') }}</th>
                    <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('common.zone') }}</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($locations as $location)
                    <tr class="hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-150">
                        <td class="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $location->name }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $location->code }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $location->building ?? '—' }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $location->floor ?? '—' }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $location->zone ?? '—' }}</td>
                        ...
                        <td class="px-4 py-2 whitespace-nowrap text-right">
                            <div class="relative z-10 inline-block text-left" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.outside="open = false" x-cloak
                                    ...
                                    <a href="{{ route('settings.equipment-locations.edit', $location) }}"
                                    ...
                                    <form method="POST" action="{{ route('settings.equipment-locations.destroy', $location) }}" class="block"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('common.no_equipment_locations') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
