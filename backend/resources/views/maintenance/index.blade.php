...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ __('common.pm_am_plans') }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('common.pm_am_plans_desc') }}</p>
        </div>
        <a href="{{ route('maintenance.create-plan') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">{{ __('common.create_pm_am_plan') }}</a>
    </div>
    ...
    <div class="bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <form method="GET" action="{{ route('maintenance.index') }}" class="mb-4 flex flex-wrap items-end gap-2">
            <div>
        ...
        <div class="space-y-2">
            @forelse($myInstances as $item)
                <a href="{{ route('maintenance.show', $item) }}" class="block rounded-lg border border-gray-200 dark:border-gray-700 p-3 bg-white dark:bg-gray-900/20 hover:border-blue-400 dark:hover:border-blue-500 transition-colors">
